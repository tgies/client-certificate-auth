...
export function parseRfc9440(headerValue) {
    ...
    // RFC 8941 section 4.2.7 permits omitted padding. Synthesize it only
    // when absent; an explicitly padded value must already have valid padding.
    const padded = base64.endsWith('=') ? base64 : base64.padEnd(Math.ceil(base64.length / 4) * 4, '=');
...
export function getCertificateFromHeaders(headers, config) {
    ...
    // Header names are lowercase throughout: Node lowercases what it parses,
    // presets declare them that way, and a custom one is lowercased above.
