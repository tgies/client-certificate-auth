    ...
    try {
        ...
        for (const pair of pairs) {
            ...
            if (key === 'Cert' || key === 'Chain') {
                ...
                // Stryker disable next-line StringLiteral,MethodExpression,LogicalOperator: quote stripping is cosmetic; URL-encoded PEM between quotes decodes and parses identically with or without surrounding " chars
...
export function parseBase64Der(headerValue) {
    ...
    // Stryker disable next-line MethodExpression,ArrowFunction: .trim() no-op (base64 ignores whitespace); .filter(Boolean) redundant (empty strings throw in X509Certificate, caught below); arrow fn mutation → undefined filtered by .filter(Boolean)
