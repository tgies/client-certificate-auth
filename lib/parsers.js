...
export function parseUrlPem(headerValue) {
    // Stryker disable next-line ConditionalExpression,BlockStatement: falsy input falls through to try-catch → same null return
    if (!headerValue) {
        return null;
    }
...
export function parseXfcc(headerValue) {
    ...
    try {
        ...
        for (const pair of pairs) {
            ...
            // Stryker disable next-line all: empty key from substring(0,-1) can't match Cert/Chain
            if (eqIndex === -1) {
                continue;
            }
            ...
            if (key === 'Cert' || key === 'Chain') {
                ...
                // Stryker disable next-line all: mutations still return null via cert parse failure
...
export function parseBase64Der(headerValue) {
    ...
    // Stryker disable next-line all: single cert parses without split; empty strings throw in X509Certificate (caught); base64 ignores whitespace
    const certParts = headerValue.split(',').map(s => s.trim()).filter(Boolean);

    // Stryker disable next-line all: redundant with certs.length === 0 check below
    ...
    const certs = certParts.map(base64 => {
        try {
            ...
            // Stryker disable next-line BlockStatement: undefined vs null both filtered by .filter(Boolean)
            return null;
        }
    }).filter(Boolean);
...
export function parseRfc9440(headerValue) {
    ...
    try {
        ...
        // Stryker disable next-line all: Node.js base64 decoder ignores non-base64 characters like colons
...
export function getCertificateFromHeaders(headers, config) {
    ...
    // Stryker disable next-line all: Node.js HTTP always lowercases headers; headerName already lowercase from line 312 or presets
    const headerValue = headers[headerName] || headers[headerName.toLowerCase()];
    // Stryker disable next-line all: redundant with parser returning null for falsy input
    if (!headerValue) {
        return null;
    }
