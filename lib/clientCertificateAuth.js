...
export default function clientCertificateAuth(callback, options = {}) {
  ...
  return function middleware(req, res, next) {
    ...
    if (!result.success) {
      ...
      switch (result.reason) {
        case 'verification_header_mismatch':
          message = 'Unauthorized: Certificate verification failed';
          break;
        case 'header_missing_or_malformed':
          message = 'Unauthorized: Client certificate header missing or malformed';
          break;
        case 'socket_not_authorized':
          message = 'Unauthorized: Client certificate required';
          break;
        case 'certificate_not_retrievable':
          message = 'Client certificate was authenticated but certificate information could not be retrieved.';
          statusCode = 500;
          break;
      }
     ...
     */
    function doneAuthorizing(verdict) {
      if (verdict === true) {
