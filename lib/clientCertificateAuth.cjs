...
async function loadModule() {
    // Stryker disable next-line all: module caching by earlier test runs means _default is already set
    if (!_default) {
...
function clientCertificateAuth(callback, options = {}) {
    ...
    function safeCallHook(hook, ...args) {
        // Stryker disable next-line all: non-function hook handled by queueMicrotask try-catch
        if (typeof hook !== 'function') return;
        queueMicrotask(() => {
            try {
                const result = hook(...args);
                // Stryker disable next-line all: .catch() on non-Promise throws, caught by outer try-catch
