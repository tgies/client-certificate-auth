...
/**
 * Parse a certificate and the extensions used by the path check. Malformed
 * untrusted input fails closed, including invalid DER inside an extension
 * that X509Certificate accepts as an opaque OCTET STRING.
 * @param {import('./parsers.js').ChainedPeerCertificate} cert
 * @returns {PathCertificate | null}
 */
function toPathCertificate(cert) {
    try {
        return inspect(new X509Certificate(cert.raw));
    } catch {
        return null;
    }
}
 ...
 * @param {Buffer} der
 * @param {number} offset
 * @param {number} [limit] - End of the containing element, exclusive
 * @returns {{ tag: number, start: number, end: number }}
 */
function readElement(der, offset, limit = der.length) {
    if (offset + 2 > limit) {
        throw new Error(`client-certificate-auth: malformed DER header at offset ${offset}`);
    }
    ...
    if (length & 0x80) {
        ...
        // DER forbids indefinite and non-minimal lengths. Six length octets
        // already exceed Node's maximum Buffer size and keep arithmetic exact.
        if (size === 0 || size > 6 || start + size > limit || der[start] === 0) {
            throw new Error(`client-certificate-auth: malformed DER length at offset ${offset}`);
        }
        length = 0;
    ...
    if (length > limit - start) {
        throw new Error(`client-certificate-auth: malformed DER content at offset ${offset}`);
    }
...
function readExtensions(der) {
    ...
    const tbs = readElement(der, certificate.start, certificate.end);
    const extensions = [];
    let offset = tbs.start;
    while (offset < tbs.end) {
        const field = readElement(der, offset, tbs.end);
        if (field.tag === 0xa3) {
            const list = readElement(der, field.start, field.end);
            let position = list.start;
            while (position < list.end) {
                const entry = readElement(der, position, list.end);
                const oid = readElement(der, entry.start, entry.end);
                let next = readElement(der, oid.end, entry.end);
                let critical = false;
                if (next.tag === 0x01) {
                    critical = der[next.start] !== 0;
                    next = readElement(der, next.end, entry.end);
                }
                extensions.push({
...
function pathLengthConstraint(extensions) {
    ...
    while (offset < sequence.end) {
        const field = readElement(basic.value, offset, sequence.end);
        if (field.tag === 0x02) {
            let length = 0;
...
/**
 * Canonicalize a decoded Name attribute using OpenSSL's X509_NAME_cmp rules.
 * String types accepted by its Name decoder and canonicalizer share a UTF-8
 * representation with ASCII whitespace compressed and ASCII letters folded.
 * Other accepted types retain their tag and bytes.
 * Non-ASCII case and spaces stay distinct, as they do in checkIssued().
 * X509Certificate has already validated these Name strings during decoding.
 * @see https://github.com/openssl/openssl/blob/openssl-3.0/crypto/x509/x_name.c
 * @param {number} tag
 * @param {Buffer} value
 * @returns {[number, string]}
 */
function canonicalNameValue(tag, value) {
    let text;
    switch (tag) {
        ...
        case 0x16: // IA5String
            ...
            break;
        case 0x1e: // BMPString (big-endian UCS-2)
            text = Buffer.from(value).swap16().toString('utf16le');
            break;
        case 0x1c: // UniversalString (big-endian UCS-4)
            text = '';
            for (let i = 0; i < value.length; i += 4) {
                text += String.fromCodePoint(value.readUInt32BE(i));
            }
            break;
        default:
            return [tag, value.toString('hex')];
    }
    return [0x0c, text.replace(/[\t\n\v\f\r ]+/g, ' ').replace(/^ | $/g, '').replace(/[A-Z]/g, (letter) => letter.toLowerCase())];
}

/**
 * Compare the actual Name attributes, preserving RDN order and grouping.
 * Each RDN is a SET: sort its canonical attributes without sorting the RDNs.
 * Structured serialization keeps punctuation inside a value from becoming a
 * delimiter, unlike comparing Node's escaped display strings.
 * @param {Buffer} der
 * @param {{ start: number, end: number }} name
 * @returns {string}
 */
function canonicalName(der, name) {
    const rdns = [];
    let offset = name.start;
    while (offset < name.end) {
        const rdn = readElement(der, offset, name.end);
        const attributes = [];
        let position = rdn.start;
        while (position < rdn.end) {
            const entry = readElement(der, position, rdn.end);
            const oid = readElement(der, entry.start, entry.end);
            const value = readElement(der, oid.end, entry.end);
            attributes.push(JSON.stringify([
                der.subarray(oid.start, oid.end).toString('hex'),
                canonicalNameValue(value.tag, der.subarray(value.start, value.end)),
            ]));
            position = entry.end;
        }
        rdns.push(attributes.sort());
        offset = rdn.end;
    }
...
/**
 * Extract issuer and subject Names from an already-decoded certificate.
 * The first four TBSCertificate SEQUENCE children are signature algorithm,
 * issuer, validity, and subject. Version and serial number have other tags,
 * so this also handles v1 certificates without an explicit version field.
 * @param {Buffer} der
 * @returns {boolean}
 */
function isSelfIssued(der) {
    const certificate = readElement(der, 0);
    const tbs = readElement(der, certificate.start, certificate.end);
    const sequences = [];
    let offset = tbs.start;
    while (sequences.length < 4) {
        const field = readElement(der, offset, tbs.end);
        if (field.tag === 0x30) {
            sequences.push(field);
        }
        offset = field.end;
    }
    return canonicalName(der, sequences[1]) === canonicalName(der, sequences[3]);
}
...
function inspect(x509) {
    ...
    return {
        ...
        selfIssued: isSelfIssued(x509.raw),
...
export function allowCA(caCertificates, options = {}) {
    ...
    return (/** @type {import('./parsers.js').ChainedPeerCertificate} */ cert) => {
        const now = Date.now();
        let subject = toPathCertificate(cert);
        ...
        for (let depth = 0; depth < maxDepth; depth++) {
            ...
            const link = toPathCertificate(issuer);
            if (!link || !link.x509.ca || !issuedBy(subject.x509, link.x509)) {return false;}
