...
export function allowFingerprints(fingerprints) {
    ...
    // Stryker disable next-line StringLiteral: case-insensitive flag redundant after toUpperCase; normalization consistent on both sides
...
export function allowSerial(serials) {
    ...
    // Stryker disable next-line StringLiteral: normalization consistent on both sides of comparison
...
export function allowSAN(values) {
    ...
    return (cert) => {
        ...
        return entries.some((entry) => {
            ...
            // Stryker disable next-line all: full entry already checked on line above; realistic SAN types never have colon at position 1
            if (colonIdx !== -1) {
